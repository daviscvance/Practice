    ...
    def floodFill(self, image: List[List[int]], sr: int, sc: int,
        ...
        def dfs(r, c):
            if r < 0 or r >= R or c < 0 or c >= C:
                return  # Ensure were inside the matrix.
            if image[r][c] != pixel:
                return  # Current pixel is not the original color, skip.
            image[r][
                c] = color  # Update current pixel color if it was the same as origin.
