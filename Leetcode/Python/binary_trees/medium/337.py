...
class Solution:
    # # DFS Recursion | Time: O(n) | Space: O(log(n) -> n)
    # def rob(self, root: Optional[TreeNode]) -> int:
    ...
    def rob(self, root: TreeNode) -> int:
        ...
        def getLoot(node: Optional[TreeNode], is_safe_to_loot: bool = False):
            ...
            if is_safe_to_loot:
                ...
                if node in saved:
                    return saved[node]
                rob = node.val + getLoot(node.left, True) + getLoot(node.right, True)
    ...
    #         if not has_robbed_parent:  # Is not safe to rob this one, parent was already robbed.
    #             if node in looted:
    #                 return looted[node]
    #             outcome = getLoot(node.left, False) + getLoot(node.right, False)
    #             looted[node] = outcome
    #             return outcome
    #         else:  # Safe to rob this house.
    #             if node in saved:
    #                 return saved[node]
    #             loot_outcome = node.val + getLoot(node.left, True) + getLoot(node.right, True)
    #             safe_outcome = getLoot(node.left, False) + getLoot(node.right, False)

    #             outcome = max(loot_outcome, safe_outcome)
    #             saved[node] = outcome
    #             return outcome
    #     return getLoot(root, False)
