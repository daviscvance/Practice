...
class NumMatrix:
    ...
    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        ...
        return (
            self.pre_sum[row2 + 1][col2 +
                                   1]  # The full matrix summary (>r5c4 = 38).
            - self.pre_sum[row1][
                col2 + 1]  # Minus the sum submatrix above (*r2c4 = 24).
            - self.pre_sum[row2 + 1][
                col1]  # Minus the sum submatrix before (*r5c1 = 14).
            + self.pre_sum[row1][col1]
        )  # Area was double subbed, add back (~r2c1 = 8).
