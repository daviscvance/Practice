...
class Solution:
    ...
    def topKFrequent(self, nums: list[int], k: int) -> list[int]:
        freq = Counter(nums)
        return [k[0] for k in heapq.nlargest(k, freq.items(), key=lambda x: x[1])]
    ...
    def topKFrequent(self, nums: list[int], k: int) -> list[int]:
        ...
        for key, freq in frequencies.items():  # Build a min heap of the top k frequencies.
            if len(heap) == k:  # Keep the size to k elements.
                heapq.heappushpop(heap, (freq, key))  # Push top freq, pop minimum.
